...
export function EmailRequest({ purpose = "pro" }: { purpose?: "pro" | "billing" }) {
  ...
  return <section className="rounded-2xl border border-brand-100 bg-brand-50 p-5">
    <h2 className="text-xl font-semibold text-navy">{billing ? "Confirm your purchase email" : "Log in with your email"}</h2>
    <p className="mt-2 text-sm leading-6 text-muted">{billing ? "Enter the email you used to pay. We will email a secure link to manage your subscription, cancel renewal, or request a refund here. No password or Paddle login needed." : "Enter the email you used to pay. We will send you a one-time login link. No password needed."}</p>
    {status === "sent" ? <div role="status" className="mt-4">
      <p className="font-semibold text-navy">Check your inbox</p>
      <p className="mt-2 text-sm leading-6 text-muted">Your {billing ? "secure link" : "login link"} is on its way to <strong>{email.trim()}</strong>. {billing ? "Open it to manage your subscription." : "Open it on the device where you want to use Pro."} The link works once and expires in 15 minutes.</p>
      <p className="mt-2 text-sm leading-6 text-muted">Allow a few minutes and check spam. {billing ? "Nothing is canceled until you confirm on the website." : "We will check your purchase when you open the link."}</p>
      ...
      <Button type="submit" disabled={status === "working"}>{status === "working" ? "Sending…" : billing ? "Email me a billing link" : "Email me a login link"}</Button>
...
export function EmailVerify() {
  ...
  return <div className="mx-auto max-w-xl px-5 py-16">
    ...
    {status === "done" ? <><p role="status" className="mt-4 text-muted">Pro is active in this browser. You have not been charged again. To log out later, open Log in from the menu.</p><ButtonLink href="/#tool" className="mt-6">Start rewriting</ButtonLink></> : <>
      ...
      {status === "no_active_purchase" && <p role="alert" className="mt-4">We could not find an active Pro purchase for this email. Try the email used at checkout. If you paid in the last 24 hours, you can also use the purchase code from your receipt. If you recently paid, do not pay again; contact support for help.</p>}
      ...
      <Link href="/login" className="mt-6 inline-flex min-h-11 items-center font-semibold text-brand-700 underline">Request a new link or get help</Link>
    </>}
  </div>;
}
