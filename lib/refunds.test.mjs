...
function fixture({ transactions = [purchase], customerEmail = email, adjustments = [], failCancel = false, failRefund = false, lostResponse = false, storageDown = false } = {}) {
  ...
  return { request, store, posts, adjustments, canceled: () => canceled, read: () => api.getRefundView(email, price, store, request, now), submit: (id = txid) => api.requestLatestPaymentRefund(email, id, price, store, request, now) };
}

test("eligible verified latest payment refunds only its full transaction after stopping renewal", async () => {
...
test("a renewal receives its own 14-day guarantee", async () => {
  ...
  assert.equal((await f.submit()).state, "pending_approval");
});
